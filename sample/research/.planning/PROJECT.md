...
With no roadmap there is no Roadmap row, so the Project row is what reaches
`PROJECT.md` and `REQUIREMENTS.md`. Once a `ROADMAP.md` lands, the Roadmap row
takes over and the Project row disappears.

## Goals

- A workspace with root docs, research docs, and nothing else.
- No `ROADMAP.md`, no `phases/`, no `STATE.md`.
